...
export default function SignupPage() {
  ...
  const onSubmit = async (data: SignupFormData) => {
    try {
      setIsLoading(true);
      setError('');
      await signup(data.email, data.password, data.name, parseInt(data.birthYear));
    ...
    <div className="min-h-screen flex items-center justify-center px-4 py-12">
      <div className="w-full max-w-md">
        <div className="text-center mb-8">
          <h1 className="text-3xl font-bold text-primary">StyleMatch</h1>
          <p className="text-text-secondary mt-2">AI美容診断で理想の美容師と出会う</p>
        </div>
            ...
            <CardContent className="space-y-4">
              ...
              <div>
                <label htmlFor="birthYear" className="block text-sm font-medium mb-1">
                  生年 <span className="text-error">*</span>
                </label>
                <select
                  id="birthYear"
                  {...register('birthYear')}
                ...
                {errors.birthYear && (
                  <p className="text-error text-sm mt-1">{errors.birthYear.message}</p>
                )}
              </div>
